...
namespace Maticsoft.Web.Ashx
{
    ...
    /// ProductSeries 的摘要说明
    /// </summary>
    public class ProductSeries : IHttpHandler, IRequiresSessionState
    {
        ...
        private string Add(HttpContext context)
        {
            try
            {
                return new BLL.ProductSeries().Add(GetProductSeries(context)) > 0 ? "ok" : "保存失败。";
            }
            catch (Exception exc)
            {
                return exc.Message;
        ...
        private string Update(HttpContext context)
        {
            try
            {
                return new BLL.ProductSeries().Update(GetProductSeries(context)) ? "ok" : "保存失败。";
            }
            catch (Exception exc)
            {
                return exc.Message;
        ...
        private string Dels(HttpContext context)
        {
            try
            {
                int count = new BLL.ProductSeries().DeleteList(context.Request.Params["cbx_select"]);
        ...
        private Model.ProductSeries GetProductSeries(HttpContext context)
        {
            Model.ProductSeries st = new Model.ProductSeries();
