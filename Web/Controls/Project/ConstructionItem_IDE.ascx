<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ConstructionItem_IDE.ascx.cs" Inherits="Maticsoft.Web.Controls.Project.ConstructionItem_IDE" %>
...
<script type="text/javascript">
    ...
    function my97date() {
        ...
        var curr_row = $('#ft').datagrid('getSelected');
        if (curr_row != null) {
            index = $('#ft').datagrid('getRowIndex', curr_row);
            if (index == 0 || curr_row.s_sid == "" || curr_row.s_sid == undefined) {
                sDateTmp = $('#citem_begin_date').val();
                eDateTmp = $('#citem_end_date').val();
            } else {
                sDateTmp = rows[index - 1].end_time;
                eDateTmp = $('#citem_end_date').val();
            }
        }
        WdatePicker({ dateFmt: 'yyyy-MM-dd', minDate: sDateTmp, maxDate: eDateTmp, onpicked: function () { ChangeDate(index) } });
    }

    function ChangeDate(index) {
        var rows = $('#ft').datagrid('getRows');
        if (rows.length > index && rows[index].s_sid != "" && rows[index].s_sid != undefined) {
            var ed = $('#ft').datagrid('getEditor', { index: index, field: 'end_time' });
            //$(ed.target).datebox('setValue', '5/4/2012');
            if ($(ed.target).val() != "") {
                $('#ft').datagrid('updateRow', {
                    index: index + 1,
                    row: {
                        begin_time: $(ed.target).val()
                    }
                });
            }
        }
    ...
    $.extend($.fn.datagrid.defaults.editors, {
        my97date: {
            init: function (container, options) {
                ...
                var input = $('<input type="text" onclick="my97date()" class="' + fmt + '">').appendTo(container);
    ...
    function initFTGrid(pssid, psisid) {
        $('#ft').datagrid({
            ...
            queryParams: { "action": "flow_list", "ps_sid": pssid, "psi_sid": psisid, "type": citem_editType },
            ...
            { title: '删除', field: 'del', align: 'center', width: 30, hidden: true, formatter: function (value, rowData, rowIndex) { if (rowData.s_sid == "" || rowData.s_sid == undefined) return '<a  href=\'#\' onclick=\'citem_flow_del(' + rowIndex + ');\'><img src=\'../images/0.png\' /></a>'; else return ''; } },
                    ...
                    { title: '开始时间', field: 'begin_time', width: 80, sortable: true, halign: 'center', align: 'center', editor: "my97date"
                        ,
                        formatter: function (value, rowData, rowIndex) {
                    ...
                    { title: '结束时间', field: 'end_time', width: 80, sortable: true, halign: 'center', align: 'center', editor: "my97date"
                        ,
                        formatter: function (value, rowData, rowIndex) {
                            if (value != '' && value != undefined) {
                                return value.Format("yyyy-MM-dd");
...
<%--        <tr id="tr_group">
       ...
       <tr>
            ...
            <td style="background-color: #ffffff; height: 25px; padding-left: 5px;">
                <input type="text" id="citem_begin_date" name="citem_begin_date" value="" style="width: 178px"
                    onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd', minDate: sDateStr, maxDate: $('#citem_end_date').val()==''?eDateStr: $('#citem_end_date').val(),onpicked:function(){ChangeSDate()}});" class="Wdate"" />
            </td>
        </tr>
        <tr>
            <td align="right" style=" background-color: #e1f5fc; height: 25px;">
            ...
            <td style="background-color: #ffffff; height: 25px; padding-left: 5px;">
                <input type="text" id="citem_end_date" name="citem_end_date" value="" style="width: 178px" onfocus="WdatePicker({ dateFmt: 'yyyy-MM-dd', minDate: $('#citem_begin_date').val()==''?sDateStr: $('#citem_begin_date').val(), maxDate: eDateStr ,onpicked:function(){ChangeEDate()}});"
    ...
    <table id="ft" style="width:100%; height: 240px;" ></table>
    <%--<table>
    <tr>
            <td align="right" style=" background-color: #e1f5fc; height: 25px;">
                工作类型：
            ...
            <td style="background-color: #ffffff; height: 25px; padding-left: 5px;">
                <select id="citem_p_imp" style="width: 180px;">
                </select>
                <table>
                </table>
            </td>
            ...
            <td style="background-color: #ffffff; height: 25px; padding-left: 5px;">
                <select id="citem_p_imp_user" style="width: 160px;">
                </select><input type="button" id="citem_bt_add" onclick="citem_addUser();" value="+" />
            </td>
        </tr>
        <tr>
    ...
    <div>
    <table id="citem_imp_user_tt" class="easyui-datagrid" style="width:100%; height: 115px;" data-options="url:'',singleSelect:true,width:700,toolbar: [{
                iconCls: 'icon-add',
                ...
                text: '工作种类入人员设置'
                    ...
                    <thead>
                        <tr>
                            ...
                            <th data-options="field:'wusers',width:300">
                                工作人员
                            </th>
                            <th data-options="field:'implementer',width:180">
                                人员简称
                            </th>
