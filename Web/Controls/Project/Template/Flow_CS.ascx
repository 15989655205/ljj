<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Flow_CS.ascx.cs" Inherits="Maticsoft.Web.Controls.Project.Template.Flow_CS" %>
<script type="text/javascript">
    ...
    function flow_Grid(row) {
        $('#flow_tt').datagrid({
            ...
            queryParams: { "action": "flow_list", "ssid": row.sid },
            idField: 'sid',
            fit: true,
    ...
    function flow_EditRow(index) {
        if (flow_editIndex != index) {
            if (flow_endEditing()) {
                ...
                var editor = $('#flow_tt').datagrid('getEditor', { index: index, field: 'work_flow_name' });
    ...
    function flow_MoveRow(fx) {
        ...
        var editor = $('#flow_tt').datagrid('getEditor', { index: iMovedRowIndex, field: 'work_flow_name' });
        $(editor.target).attr('disabled', true);
