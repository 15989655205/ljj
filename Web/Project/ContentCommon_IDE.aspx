<%@ Page Title="" Language="C#" MasterPageFile="~/easyui.Master" AutoEventWireup="true"
    CodeBehind="ContentCommon_IDE.aspx.cs" Inherits="Maticsoft.Web.Project.ContentCommon_IDE" %>

<%@ Register TagPrefix="pc_IDE" TagName="PC_IDE" Src="../Controls/Project/Content_IDE.ascx" %>
<%@ Register TagPrefix="pp_IDE" TagName="PP_IDE" Src="../Controls/Project/ProjectProduct.ascx" %>
...
<link href="../js/themes/default/datagrid2.css" rel="stylesheet" type="text/css" />
    ...
    <script type="text/javascript">
        ...
        var pssid = "<%=pssid %>";
        var isConstruction = "<%=isConstruction %>";
        ...
        function InitGird() {
            grid = $('#tt').datagrid({
                ...
                queryParams: { "action": "content_common_item", "pssid": "<%=pssid %>", "isConstruction": "<%=isConstruction %>" },
           ...
           , pageList: [10, 20, 30, 40, 50, 100]
            ...
            , frozenColumns: eval('(' + frozencolumns + ')'),
                ...
                onLoadSuccess: function (data) {
                    ...
                    if('<%=isConstruction %>'=='3') mergeCellsByField('tt', 'number,productName,productPic,Specifications,paintColor,useSpace,spaceCount,install,usePart,unit,amount,paintPaletteNumber,ppiRemark,stage_name,group_name,contentName');  else mergeCellsByField('tt', 'stage_name,group_name,contentName');
                },
        ...
        function MoveRow(fx) {
            ...
            if (endEditing()) {
                ...
                var iCurRowIndex = gd.datagrid('getRowIndex', FocusRow); //返回指定行的索引号，该行的参数可以是一行记录或一个ID字段值。
                iMovedRowIndex = iCurRowIndex + fx;//判断是上移还是下移,赋值移动行索引
        ...
        function delContent() {
            ...
            if (isConstruction != "3") {
                ...
                $.messager.confirm('提示', '确认删除？', function (r) {
                    if (r) {
                        $.post('../../Ashx/Project.ashx', { "action": "content_del", "sid": row.parent_sid }, function (data) {
                            if (data == "success") {
                                $("#tt").datagrid("clearSelections");
                                $("#tt").datagrid("reload");
                            }
            ...
            if (row != null) {
                if (endEditing()) {
                    ...
                    if (isConstruction != "3") {
                        ...
                        $('#tt').datagrid('insertRow', { index: index + 1, row: {
                            number: row.number, productName: row.productName, remark: row.remark, productPic: row.productPic, Specifications: row.Specifications, paintColor: row.paintColor,
                            useSpace: row.useSpace, spaceCount: row.spaceCount, install: row.install, usePart: row.usePart, unit: row.unit, amount: row.amount,
                            paintPaletteNumber: row.paintPaletteNumber, contentName: row.contentName, sid: '', ppiSid: row.ppiSid, PP_sid: row.PP_sid, parentID: row.parentID, csid: row.parent_sid, itemSequence: row.itemSequence + 1
                        }
                        });
                        //editIndex = $('#tt').datagrid('getRows').length - 1;
                        editIndex = index + 1;
        ...
        //                    number: row.number, productName: row.productName, remark: row.remark, productPic: row.productPic, Specifications: row.Specifications, paintColor: row.paintColor,
        //                    useSpace: row.useSpace, spaceCount: row.spaceCount, install: row.install, usePart: row.usePart, unit: row.unit, amount: row.amount, 
        //                     paintPaletteNumber: row.paintPaletteNumber, csid: row.csid, itemSequence: row.itemSequence + 1
        //                }
        //                });
        //                //editIndex = $('#tt').datagrid('getRows').length - 1;
        //                editIndex = index + 1;
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div id="toolbar" style="display: none;">
        <a href="javascript:void(0)" id="mb" class="easyui-menubutton" data-options="menu:'#mm',iconCls:'icon-edit'">
            <%=showContent %></a>
        ...
        <a href="#" id="tab_a_01" class="easyui-linkbutton" plain="true" onclick="add()"
            iconcls="icon-add">添加<%=showItem%></a> <a href="#" id="A2" class="easyui-linkbutton"
                plain="true" onclick="Item_del()" iconcls="icon-remove">删除<%=showItem%></a>
        <a href="#" id="A3" class="easyui-linkbutton" plain="true" onclick="MoveRow(-1)"
            iconcls="icon-up">上移</a> <a href="#" id="A4" class="easyui-linkbutton" plain="true"
                onclick="MoveRow(1)" iconcls="icon-down">下移</a> <a href="#" id="A5" class="easyui-linkbutton"
                    plain="true" onclick="save()" iconcls="icon-save">保存</a> <a href="#" id="A6" class="easyui-linkbutton"
    ...
    <div id="pp">
        <pp_IDE:PP_IDE ID="pp_ide1" runat="server"></pp_IDE:PP_IDE>
    </div>
</asp:Content>
