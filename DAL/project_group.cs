...
namespace Maticsoft.DAL
{
	...
	public partial class project_group
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("sid", "project_group"); 
		}
		...
		public bool Exists(int sid)
		{
			...
			strSql.Append("select count(1) from project_group");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Maticsoft.Model.project_group model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into project_group(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Maticsoft.Model.project_group model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update project_group set ");
		...
		public bool Delete(int sid)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from project_group ");
			strSql.Append(" where sid=@sid");
			SqlParameter[] parameters = {
					new SqlParameter("@sid", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.project_group GetModel(int sid)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 sid,deptid,group_name,ps_sid,show_column,remark,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,n1,n2,n3,show,show_name,status,create_person,create_date,update_person,update_date from project_group ");
			strSql.Append(" where sid=@sid");
			SqlParameter[] parameters = {
					new SqlParameter("@sid", SqlDbType.Int,4)
			};
			...
			Maticsoft.Model.project_group model=new Maticsoft.Model.project_group();
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.project_group DataRowToModel(DataRow row)
		{
			Maticsoft.Model.project_group model=new Maticsoft.Model.project_group();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select *,dbo.getUserName_fu(v1) v1_name ");
			strSql.Append(" FROM project_group ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from project_group T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "project_group";
        ...
        public bool Edit(List<Model.project_group> insertList, List<Model.project_group> updateList, List<Model.project_group> delList, List<Model.project_group> sequenceList)
        {
            ...
            for (int i = 0; i < delList.Count; i++)
            {
                ...
                Model.project_group model = delList[i];
                sbSql.Append("delete from project_group where sid=" + model.sid);
            ...
            {
                sbSql = new StringBuilder();
                Model.project_group model = updateList[i];
                sbSql.Append("update project_group set group_name = '" + model.group_name.Trim() + "',");
            ...
            for (int i = 0; i < insertList.Count; i++)
            {
                ...
                sbSql = new StringBuilder();
                Model.project_group model = insertList[i];
                if (string.IsNullOrEmpty(model.v1))
                {
                    model.v1 ="''";
                }
                sbSql.Append("insert into project_group(group_name, remark,ps_sid,sequence,v1");
                sbSql.Append(")values(");
                sbSql.Append("'" + model.group_name.Trim() + "','" + model.remark.Trim() + "','" + model.ps_sid + "'," + model.sequence+","+model.v1);
                sbSql.Append(")");
                al.Add(sbSql.ToString());
            }
            ...
            {
                sbSql = new StringBuilder();
                Model.project_group model = sequenceList[i];
                if (model.sid != null && model.sid.ToString().Trim() != "0" && model.sid.ToString().Trim() != "")
                {
                    sbSql.Append("update project_group set sequence = " + model.sequence.ToString().Trim());
                    sbSql.Append(" where sid = " + model.sid.ToString().Trim() + "");
                    al.Add(sbSql.ToString());
                }
            }
