...
namespace Maticsoft.DAL
{
	...
	public partial class stage
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("sid", "stage"); 
		}
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Maticsoft.Model.stage model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into stage(");
			strSql.Append("stage_name,p_sid,show_column,begin_date,end_date,remark,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,show,show_name,status,create_person,create_date,update_person,update_date)");
			...
			parameters[16].Value = model.show;
			parameters[17].Value = model.show_name;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Maticsoft.Model.stage model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.stage GetModel(int sid)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 sid,stage_name,p_sid,show_column,begin_date,end_date,remark,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,show,show_name,status,create_person,create_date,update_person,update_date from stage ");
			strSql.Append(" where sid=@sid");
			SqlParameter[] parameters = {
					new SqlParameter("@sid", SqlDbType.Int,4)
			};
			...
			Maticsoft.Model.stage model=new Maticsoft.Model.stage();
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.stage DataRowToModel(DataRow row)
		{
			Maticsoft.Model.stage model=new Maticsoft.Model.stage();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select sid,stage_name,p_sid,show_column,begin_date,end_date,remark,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,show,show_name,status,create_person,create_date,update_person,update_date ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" sid,stage_name,p_sid,show_column,begin_date,end_date,remark,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,show,show_name,status,create_person,create_date,update_person,update_date ");
			strSql.Append(" FROM stage ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
        ...
        public bool Edit(List<Model.stage> insertList, List<Model.stage> updateList, List<Model.stage> delList, List<Model.stage> sequenceList,string uid)
        {
            ...
            for (int i = 0; i < delList.Count; i++)
            {
                ...
                Model.stage model = delList[i];
                sbSql.Append("delete from stage where sid=" + model.sid);
            ...
            {
                sbSql = new StringBuilder();
                Model.stage model = updateList[i];
                sbSql.Append("update stage set stage_name = '" + model.stage_name.Trim() + "',");
                sbSql.Append("status = '" + model.status.ToString().Trim() + "',");
                sbSql.Append("sequence = '" + model.sequence.ToString().Trim() + "',");
                sbSql.Append("update_person = '" + uid + "',");
                sbSql.Append("update_date = getdate(),");
                sbSql.Append("is_system = '" + model.is_system.ToString().Trim() + "',");
            ...
            {
                sbSql = new StringBuilder();
                Model.stage model = insertList[i];
                sbSql.Append("insert into stage(stage_name,status, remark,sequence,is_system,create_person");
                sbSql.Append(")values(");
                sbSql.Append("'" + model.stage_name.Trim() + "','" + model.status.ToString().Trim() + "','" + model.remark.Trim() + "'," + model.sequence+","+model.is_system+",'"+uid+"'");
                sbSql.Append(")");
                al.Add(sbSql.ToString());
            }
            ...
            {
                sbSql = new StringBuilder();
                Model.stage model = sequenceList[i];
