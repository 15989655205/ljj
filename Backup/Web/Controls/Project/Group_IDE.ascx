<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Group_IDE.ascx.cs" Inherits="Maticsoft.Web.Controls.Project.Group_IDE" %>
<script type="text/javascript">
    ...
    function group_Grid(row) {
        $('#group_tt').datagrid({
            ...
            columns: [[
                    ...
                    , { field: 'v1_name', title: '组长', width: 200, sortable: true, editor: "text", editor: "onclick_text1" }
					, { field: 'remark', title: '备注', width: 200, sortable: true, editor: "text" }
                    , { field: 'v1', title: '', editor: "text", hidden: true }
                    , { field: 'sid', title: '编号', editor: "text", hidden: true }
                    , { field: 'sequence', title: '排序', hidden: true }
                    
                    
            //,{ field: 'ps_sid', title: '父编号', width: 100, sortable: true, editor: "text",hidden:true }
				]],
            toolbar: "#group_menu",
    ...
    function group_EditRow(index) {
        if (group_editIndex != index) {
            if (group_endEditing()) {
                ...
                var edt = $('#group_tt').datagrid('getEditors', index);
    ...
    function openSelectWin1() {
        $('#group_box').modal({ zIndex: 9001, maxHeight: 160, maxWidth: 300, minHeight: 160, minWidth: 300 });
        group_dept_tree_single();
        $('#group_select_bt').focus();
    }

    function gropu_setSelectUser() {
        var row = $('#group_tt').datagrid('getSelected');
            var edt = $('#group_tt').datagrid('getEditors', group_editIndex);
            //alert(edt.length);
            edt[1].target.val($('#group_uid').combobox('getText'));
            //edt[2].target.val($('#group_un').val());
            edt[3].target.val($('#group_uid').combobox('getValues'));
            group_btnSave();
            $.modal.close();
    }

    function group_dept_tree_single() {
        $('#group_dept').combotree({
            url: '../Ashx/Common.ashx?type=dept_tree_open',
            valueField: 'id',
            textField: 'text',
            editable: false,
            onLoadSuccess: function (node, data) {
                $('#group_dept').combotree('tree').tree('collapseAll');
                group_user_single();
            },
            onChange: function (newValue, oldValue) {
                $.post("../Ashx/Common.ashx", { type: "dept_users", deptid: newValue }, function (data) {
                    group_push_single(data);
                });
            }
        });
    }

    function group_push_single(data) {
        var array = JSON.parse(data);
        var rows = $('#group_uid').combobox('getValue');
        var textArr = $('#group_uid').combobox('getText'); 
        var ishave = false;
        for (var j = 0; j < array.length; j++) {

            if (array[j].UserName == rows) {
                ishave = true;
                break;
            }
        }
        if (!ishave && rows != "") {
            array.push({ "UserName": rows, "Name": textArr });
        }
        $('#group_uid').combobox('loadData', array);
    }

    function group_user_single() {
        var myload = false;
        $('#group_uid').combobox({
            url: '',
            valueField: 'UserName',
            ...
            onLoadSuccess: function () {
                if (!myload) {
                    var row = $('#group_tt').datagrid('getSelected');
                    $(this).combobox('setValue', row.grouplementers_sid);
                    myload = true;
                }
            },
            onChange: function (newValue, oldValue) {
                var values = $('#group_uid').combobox('getValue');
                if (values.length > 0) {
                    $.ajax({
                        type: "POST",
                        timeout: 30000, //超时时间：30秒
                        url: '../Ashx/Project.ashx?action=short',
                        data: {
                            'sql': values
                        },
            ...
            art.dialog({
                ...
                close: function () {
                    ...
                    if (changerows.length > 0) {
                        ...
                        //                            if (r) {
                        //                                // exit action;
                        //                                art.dialog.list['group_dialog'].hide();
                        //                            }
                        //                            else {
                        //                                //return false;
                        //                            }
...
//            });
    ...
    function MoveRow(fx) {
        ...
        RowTemp.v1 = gdData.rows[iMovedRowIndex].v1;
        RowTemp.v1_name = gdData.rows[iMovedRowIndex].v1_name;
        ...
        gdData.rows[iMovedRowIndex].v1 = gdData.rows[iCurRowIndex].v1; ;
        gdData.rows[iMovedRowIndex].v1_name = gdData.rows[iCurRowIndex].v1_name;
        
        //gdData.rows[iMovedRowIndex].sequence = gdData.rows[iCurRowIndex].sequence;
        //gdData.rows[iCurRowIndex].sid = RowTemp.sid;
        var edt = gd.datagrid("getEditors", iCurRowIndex);
        edt[0].target.val(RowTemp.group_name);
        edt[1].target.val(RowTemp.v1_name);
        edt[2].target.val(RowTemp.remark);
        edt[3].target.val(RowTemp.sid);
        edt[4].target.val(RowTemp.v1);
        //edt[3].target.val(RowTemp.sid);
...
<div style="width:600px; height:400px;">
 <div id="group_menu" style="padding:2px;border:1px solid #ddd; background-color:#F4F4F4">
		<a href="#" id="group_add" onclick="group_add()" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">新增</a>
        ...
        <a href="#" id="group_up" onclick="MoveRow(-1)" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-up'">上移</a>
        <a href="#" id="group_down" onclick="MoveRow(1)" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-down'">下移</a>
...
<div id="group_box" style="z-index:9999">
<div style="padding:5px;">部&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;门：<input type="text" id="group_dept" name="group_dept" value="" style="width:200px; z-index:99999"/><br /></div>
<div style="padding:5px">人&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;员：<input type="text" id="group_uid" name="group_uid" value="" style="width:200px"/><br /></div>
<div style="padding:5px">人员简称：<input type="text" id="group_un" name="group_un" value="" style="width:200px"/><br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;简称顺序必须与上面人员相同</div>
<div style="padding:5px; text-align:center">
<%--<a href="#" id="group_select_bt" onclick="" class="easyui-linkbutton" data-options="plain:false,iconCls:'icon-save'">确定</a>--%>
<button id="group_select_bt" onclick="gropu_setSelectUser();">确定</button>
</div>
</div>
