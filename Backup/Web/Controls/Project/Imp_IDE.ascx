<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Imp_IDE.ascx.cs" Inherits="Maticsoft.Web.Controls.Project.Imp_IDE" %>
<script type="text/javascript">
        ...
        $('#imp_left_tt').datagrid({
            ...
            onClickRow: function (rowIndex, rowData) {
                ...
                if (!ishave) {
                    ...
                    $('#imp_tt').datagrid('appendRow', { s_sid: imp_pssid, implement_name: rowData.implement_name, impUserNames: '', implementers: '', implementers_sid: '', remark: '', sequence: $('#imp_tt').datagrid('getRows').length, sid: 0 });
                }
            }
    ...
    function imp_Grid(row) {
        $('#imp_tt').datagrid({
            ...
            columns: [[
					...
					, { field: 'v1', title: '工作流程', width: 200, hidden: flag,
					    editor: { type: 'combobox', options: { data: comboboxData, valueField: 'sid', editable: false, textField: 'work_flow_name', anelHeight: "auto", multiple: true,
					        onHidePanel: function () {
					            if (imp_editIndex!=undefined) {
					                var edt = $('#imp_tt').datagrid('getEditors', imp_editIndex);
					                edt[6].target.val($(this).combobox('getText'));
					            }
                        ...
                        , formatter: function (value, row, index) {
                            ...
                            //                                var text = comboboxData[i].work_flow_name;
                            //                                if (comboboxData[i].sid == value) return text;
                            //                            }
                            //                            //return '';
                        }
					}
                    ...
                    , { field: 'fw_name', title: '', editor: "text",hidden:true }
				]],
            toolbar: "#imp_menu",
    ...
    function imp_endEditing() {
        ...
        if ($('#imp_tt').datagrid('validateRow', imp_editIndex)) {
            var ed = $('#imp_tt').datagrid('getEditor', { index: imp_editIndex, field: 'v1' });
    ...
    function imp_EditRow(index) {
        if (imp_editIndex != index) {
            if (imp_endEditing()) {
                ...
                var edt = $('#imp_tt').datagrid('getEditors', index);
            ...
            art.dialog({
                ...
                init: function () {
                    ...
                    $.ajax({
                        dataType: 'json',
                        async: false,
                        url: '../Ashx/Project.ashx?action=stage_fw&s_sid=' + row.sid,
                        timeout: 3000,
    ...
    function openSelectWin() {
        ...
        //imp_dept_tree();
        //imp_user_single();
        imp_dept_tree_single();
        //$('.combo-text').focus();
        $('#imp_select_bt').focus();
        //$('input[name="imp_uid"]').focus();
    }

    function imp_dept_tree_single() {
        $('#imp_dept').combotree({
    ...
    function imp_user() {//sid) {
        ...
        $('#imp_uid').combobox({
            ...
            onHidePanel: function () {
                var values = $('#imp_uid').combobox('getValues');
                var s = '';
                if (values.length > 0) {
    ...
    function imp_push_single(data) {
        var array = JSON.parse(data);
        var rows = $('#imp_uid').combobox('getValue');
        var textArr = $('#imp_uid').combobox('getText');//.split(',');
        //for (var i = 0; i < rows.length; i++) {
            var ishave = false;
            for (var j = 0; j < array.length; j++) {
    ...
    function imp_user_single() {//sid) {
        ...
        $('#imp_uid').combobox({
            ...
            onLoadSuccess: function () {
                if (!myload) {
                    var row = $('#imp_tt').datagrid('getSelected');
                    $(this).combobox('setValue', row.implementers_sid);
                    myload = true;
//                    myload = true;
//                    var row = $('#imp_tt').datagrid('getSelected');
//                    imp_push_json($('#imp_uid').combobox('getData'), row.implementers_sid, row.impUserNames);
            ...
            onChange: function (newValue, oldValue) {
                //$.messager.show({title:'',msg:newValue+":"+oldValue});
                //$('#imp_un').val($('#imp_uid').combobox('getText'));
                var values = $('#imp_uid').combobox('getValue');
                if (values.length > 0) {
                    $.ajax({
                        type: "POST",
                        timeout: 30000, //超时时间：30秒
...
//                }
    ...
    function imp_MoveRow(fx) {
        ...
        var RowTemp = { implement_name: "", impUserNames: "", implementers: "", v1: "",fw_name:""};
        ...
        RowTemp.fw_name = gdData.rows[iMovedRowIndex].fw_name;
        var edt = gd.datagrid("getEditors", iCurRowIndex);
        gdData.rows[iMovedRowIndex].implement_name = edt[0].target.val(); //gdData.rows[iCurRowIndex].implement_name;
        gdData.rows[iMovedRowIndex].impUserNames = edt[1].target.val(); //gdData.rows[iCurRowIndex].impUserNames;
        gdData.rows[iMovedRowIndex].implementers = edt[2].target.val(); //gdData.rows[iCurRowIndex].implementers;
        gdData.rows[iMovedRowIndex].implementers_sid = edt[5].target.val(); //gdData.rows[iCurRowIndex].implementers_sid;
        gdData.rows[iMovedRowIndex].v1 = edt[3].target.combobox('getValues').join(','); //gdData.rows[iCurRowIndex].v1;
        gdData.rows[iMovedRowIndex].sid = edt[4].target.val(); //gdData.rows[iCurRowIndex].sid;
        gdData.rows[iMovedRowIndex].fw_name = edt[6].target.val();
...
//        gdData.rows[iCurRowIndex].sid = RowTemp.sid
        ...
        edt[3].target.combobox('setValues',RowTemp.v1.split(','));
        edt[4].target.val(RowTemp.sid);
        edt[5].target.val(RowTemp.implementers_sid);
        edt[6].target.val(RowTemp.fw_name);
    ...
    $.extend($.fn.datagrid.defaults.editors.combobox, {
        ...
        setValue: function (jq, value) {
            ...
            if (opts.multiple && value.indexOf(opts.separator) != -1) {//多选且不只一个值
                ...
                $(jq).combobox("setValues", values.split(','));
            }
            else
